...
def resume_write(request):
    ...
    collection_skill=request.db['skill_set']
    for i in range (0,int(no_of_skill)):
       ...
       try:
           ...
           try: 
             ...
             existing_skill_id=skill_excists["skid"]
             skills.append(existing_skill_id)
           except KeyError:
             skillid=collection_shoolid.findone({'name':'skid'}) #obtaining the last value of skills to append it to the list
             skill_count=int(skillid["value"])
             if not skill_count:
                 skill_count=0
             skills.append(skill_count+1)
             collection_skill.update({'skid':skills[i]},{"$set":{'name_of_skill':name_of_skills[i]}},upsert=True)
             collection_schoolid.update({'name':'skid'},{"$set":{'value':skill_count+1}})
           except TypeError:
             skillid=collection_schoolid.find_one({'name':'skid'}) #obtaining the last value of skills to append it to the list
             skill_count=int(skillid["value"])
             if not skill_count:
                 skill_count=0
             skills.append(skill_count+1)
    ...
    'from_company':from_company,'to_company':to_company,'pos_company':pos_company,'no_of_emp':no_of_emp,'no_of_skill':no_of_skill,'name_skill':name_of_skills}
